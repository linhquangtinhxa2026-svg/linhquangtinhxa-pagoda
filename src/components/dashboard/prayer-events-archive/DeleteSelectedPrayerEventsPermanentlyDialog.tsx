...
import { useBulkPermanentlyDeletePrayerEvents } from "./usePrayerEventsArchive";

interface DeleteSelectedPrayerEventsPermanentlyDialogProps {
  selectedIds: string[];
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function DeleteSelectedPrayerEventsPermanentlyDialog({
  selectedIds,
  open,
  onOpenChange,
}: DeleteSelectedPrayerEventsPermanentlyDialogProps) {
  const { t } = useTranslation();
  const { doBulkPermanentlyDelete, isBulkPermanentlyDeleting } =
    useBulkPermanentlyDeletePrayerEvents({
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => !isBulkPermanentlyDeleting && onOpenChange(next)}>
      <DialogContent className="sm:max-w-sm" showCloseButton={!isBulkPermanentlyDeleting}>
        <DialogHeader>
          <DialogTitle>{t("prayerEventsArchive.deleteSelectedPermanentlyTitle")}</DialogTitle>
        </DialogHeader>
        <p className="text-sm text-gray-500">
          {t("prayerEventsArchive.deleteSelectedPermanentlyConfirm", {
